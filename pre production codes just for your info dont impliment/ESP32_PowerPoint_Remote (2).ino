...
// ---- Power saving ----
// After this many milliseconds with no joystick movement and no button
// press, the ESP32 goes into deep sleep to save battery. It wakes back
// up ONLY on a button press - joystick movement alone cannot wake it,
// since analog readings aren't a supported deep-sleep wake source on
// this chip. A button press is. Waking from deep sleep is a full
// reboot: the sketch restarts from setup() and BLE re-advertises, so
// expect a brief (roughly 1-2 second) reconnect delay after waking.
#define INACTIVITY_SLEEP_MS   60000UL   // 60 seconds idle before sleeping
...
unsigned long lastActivityTime = 0;   // updated on any joystick fire or button press
...
void handleJoystick() {
  ...
  if (xEvent == +1) { sendNextSlide(); lastActivityTime = millis(); }
  else if (xEvent == -1) { sendPreviousSlide(); lastActivityTime = millis(); }
  ...
  // rewiring anything.
  if (yEvent == +1) { sendNextSlide(); lastActivityTime = millis(); }
  else if (yEvent == -1) { sendPreviousSlide(); lastActivityTime = millis(); }
}

// ---------------------------------------------------------------------
// Puts the ESP32 into deep sleep, configured to wake only when the
// joystick button pin (PIN_JOY_SW) is pulled LOW by a press.
// This is a full chip reset on wake - execution resumes at setup(),
// not at the point sleep was entered.
// ---------------------------------------------------------------------
void goToSleep() {
  Serial.println("No activity - going to sleep. Press the joystick button to wake.");
  Serial.flush();
  delay(50);   // let the serial print actually go out before power-down

  // The pull-up configured via pinMode(INPUT_PULLUP) in setup() does
  // not persist into deep sleep's RTC domain on its own - it has to be
  // re-asserted through the RTC GPIO driver so the pin reads a clean
  // HIGH (idle) and only goes LOW on an actual press, for a reliable
  // wake trigger.
  rtc_gpio_pullup_en((gpio_num_t)PIN_JOY_SW);
  rtc_gpio_pulldown_dis((gpio_num_t)PIN_JOY_SW);

  esp_sleep_enable_ext0_wakeup((gpio_num_t)PIN_JOY_SW, 0);  // wake on LOW
  esp_deep_sleep_start();
  // Execution never returns here - on wake, the chip reboots.
}
...
void setup() {
  ...
  lastActivityTime = millis();   // start the idle timer fresh after boot/wake
}
...
void loop() {
  ...
  if (now - lastActivityTime >= INACTIVITY_SLEEP_MS) {
    goToSleep();
  }
}
